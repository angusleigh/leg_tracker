...
/**
* A simple 'local' occupancy grid map that maps everything 
* except tracked humans
*
...
private:
    ...
    /**
    * Coordinated callback for both laser scan message and a non_leg_clusters message
    ...
    */
    void laserAndLegCallback(const sensor_msgs::LaserScan::ConstPtr& scan_msg,
                             const leg_tracker::LegArray::ConstPtr& non_leg_clusters){
        ...
        if (transform_available){
            ...
            for (std::list<laser_processor::SampleSet*>::iterator c_iter = processor.getClusters().begin();
                ...
                for (std::vector<tf::Point>::iterator non_leg = non_legs.begin(); non_leg != non_legs.end(); ++non_leg){
                    ...
                    if (dist < 0.1){ // TODO: turn this hard code thd into a variable.
            ...
            if (transform_succesful){
                ...
                for (int i = 0; i < width_; i++) {
                    for (int j = 0; j < width_; j++){
                        ...
                        double angle = betweenPIandNegPI(atan2(j * resolution_ + grid_centre_pos_y_ - 
                                                            (width_ / 2.0) * resolution_ - laser_y, i * resolution_ + 
                                                            grid_centre_pos_x_ - (width_ / 2.0) * resolution_ - laser_x) - laser_yaw);
                        bool is_human;

                        if (angle > scan.angle_min - scan.angle_increment / 2.0 &&
                            ...
                            // Processing the range value of the closest_beam to
                            // determine if it's a valid measurement or not.
                            // Sometimes it returns infs and NaNs that have to be dealt
                            // with
                            ...
                            if (scan.range_min <= scan.ranges[closest_beam_idx] && scan.ranges[closest_beam_idx] <= scan.range_max) {
                                ...
                                // The sensor reported these measurements as valid, but
                                // they are discarded per the limits defined by
                            ...
                            if (valid_measurement){
                                ...
                                if (dist > scan.range_max || dist > scan.ranges[closest_beam_idx] + ALPHA / 2.0 ||
                                    fabs(angle_rel) > BETA / 2 || (!std::isfinite(scan.ranges[closest_beam_idx]) &&
                                    dist > reliable_inf_range_)){
                                        m_update = UNKNOWN;
                                }else if (scan.ranges[closest_beam_idx] < scan.range_max && fabs(dist_rel) < ALPHA / 2 && !is_human){
                                    m_update = OBSTACLE;
                                }else{
                                    m_update = FREE_SPACE;
                                }
                            }else{
                                // Assume cells corresponding to erroneous measurements
                                // are either in freespace or unknown
    ...
    /**
    * @basic The logit function, i.e., the inverse of the logistic function
    * @param p 
    ...
    */
    ...
    /**
    * @basic The inverse of the logit function, i.e., the logistic function
    * @param p 
